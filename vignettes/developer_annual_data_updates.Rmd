...
```{r}
# Save the soon to be replaced data under a new name such that the new data can take its place in the package
# Just  don't save it as package data as we don't want average user confusing it with data they should be using
# this is just an example feel free to save somewhere else, maybe an achive folder within data-raw
contraceptive_use_archived_2020 <- fpemlocal::contraceptive_use
write.csv(contraceptive_use_archived_2020, here::here("data-raw/contraceptive_use_archived_2020.csv"))
...
```{r}
# Save the soon to be replaced data under a new name such that the new data can take its place in the package
contraceptive_use_track20_archived_2020 <- fpemlocal::contraceptive_use_track20
write.csv(contraceptive_use_track20_archived_2020, here::here("data-raw/contraceptive_use_track20_archived_2020.csv"))
...
# Global estimates
```{r}
mw_perc <- fpemlocal::read_rda(here::here("data-raw/globalrun_data/global_estimates_married.rda"))
uw_perc <- fpemlocal::read_rda(here::here("data-raw/globalrun_data/global_estimates_unmarried.rda"))
...
globalrun_input_m <- readr::read_csv(here::here("data-raw/globalrun_data/data_input_married_exported_from_mcmc_meta.csv"))
globalrun_input_u <- readr::read_csv(here::here("data-raw/globalrun_data/data_input_unmarried_exported_from_mcmc_meta.csv"))
globalrun_output_m <- fpemlocal::read_rda(here::here(path = "data-raw/globalrun_data/globalrun_output_m.rda"))
globalrun_output_u <- fpemlocal::read_rda(here::here(path = "data-raw/globalrun_data/globalrun_output_u.rda"))
